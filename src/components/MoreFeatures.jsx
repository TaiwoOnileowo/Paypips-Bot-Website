    ...
    <section className="w-full ">
      <div className="grid xsm:grid-cols-2 md:grid-cols-3 xsm:px-24 justify-center gap-8 py-12 w-full">
        {moreFeatures.map((feature, index) => (
          <div
            key={feature.id}
            className={`shadow_purple jump-in w-[280px] msm:w-[350px]  hover:border hover:border-gray-300 rounded-[15px] p-6 bg-dark-bg flex gap-4 flex-col`}
          >
            <span className="text-blue-accent text-2xl msm:text-3xl ">{feature.icon}</span>
            <p className="text-base msm:text-lg">{feature.description}</p>
          </div>
        ))}
      </div>
      <a href="#features">
